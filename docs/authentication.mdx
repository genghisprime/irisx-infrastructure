...
description: 'Learn how to authenticate with the Tazzi API'
---
...
<Steps>
  <Step title="Login to Portal">
    Visit [admin.tazzi.com](https://admin.tazzi.com)
  </Step>
...
class TazziClient:
    def __init__(self, api_key=None):
        self.api_key = api_key or os.getenv('TAZZI_API_KEY')
        self.base_url = 'https://api.tazzi.com'
        self.headers = {
...
class TazziClient {
    private $apiKey;
    private $baseUrl = 'https://api.tazzi.com';

    public function __construct($apiKey = null) {
        $this->apiKey = $apiKey ?: getenv('TAZZI_API_KEY');
    }
...
<AccordionGroup>
  ...
  <Accordion title="Use Different Keys Per Environment">
    ...
    - `TAZZI_API_KEY_PRODUCTION`
    - `TAZZI_API_KEY_STAGING`
    - `TAZZI_API_KEY_DEVELOPMENT`
  </Accordion>
...
```javascript
const response = await axios.post('https://api.tazzi.com/v1/auth/register', {
...
```javascript
const response = await axios.post('https://api.tazzi.com/v1/auth/login', {
  email: 'admin@acme.com',
  password: 'SecurePassword123!'
});
...
const response = await axios.get('https://api.tazzi.com/v1/auth/me', {
  headers: {
...
async function refreshAccessToken() {
  ...
  try {
    const response = await axios.post(
      'https://api.tazzi.com/v1/auth/refresh',
...
async function logout() {
  ...
  await axios.post('https://api.tazzi.com/v1/auth/logout', {
    refresh_token: refreshToken
  });
...
```bash
# Test API Key
curl https://api.tazzi.com/v1/auth/health \
  -H "X-API-Key: your_api_key_here"
