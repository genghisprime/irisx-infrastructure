---
title: 'Node.js SDK'
description: 'Official IRISX Node.js SDK for seamless integration'
---

## Installation

Install the IRISX SDK via npm:
...
```javascript
const call = await irisx.calls.get('123e4567-e89b-12d3-a456-426614174000');
...
```javascript
const calls = await irisx.calls.list({
  status: 'completed',
  direction: 'outbound',
...
```javascript
const webhooks = await irisx.webhooks.list();
...
```javascript
const numbers = await irisx.phoneNumbers.search({
  country: 'US',
  area_code: '415',
  type: 'local',
  limit: 10
...
```javascript
await irisx.phoneNumbers.update(numberId, {
...
```javascript
const stats = await irisx.analytics.dashboard({
  start_date: '2025-10-01',
  end_date: '2025-10-31'
});
...
```javascript
const analytics = await irisx.analytics.calls({
  start_date: '2025-10-01',
  end_date: '2025-10-31',
  group_by: 'day'
});
...
All SDK errors extend `IRISXError`:

```javascript
const { IRISXError, RateLimitError, ValidationError } = require('@irisx/sdk');
...
```typescript
import IRISX, { Call, SMS, Email } from '@irisx/sdk';
...
const irisx = new IRISX({
  apiKey: process.env.IRISX_API_KEY,
  baseURL: process.env.IRISX_API_URL
});
```
