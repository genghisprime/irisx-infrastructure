...
Webhooks enable real-time event notifications from Tazzi to your application. Build robust webhook handlers that are secure, reliable, and production-ready.
  ...
  try {
    ...
    setImmediate(async () => {
      try {
        ...
        // Event will be retried by Tazzi
      }
    });
  } catch (error) {
    console.error('Webhook error:', error);
...
async function testWebhook(webhookUrl, event) {
  ...
  const response = await axios.post(webhookUrl, event, {
    headers: {
      'Content-Type': 'application/json',
      'X-Tazzi-Signature': signature,
      'X-Tazzi-Timestamp': timestamp.toString()
    }
  });
...
describe('Webhook Handlers', () => {
  ...
  test('should accept valid webhook', async () => {
    ...
    const response = await request(app)
      .post('/webhooks/calls')
      .set('X-Tazzi-Signature', signature)
      .set('X-Tazzi-Timestamp', timestamp.toString())
      .send(event);
  ...
  test('should reject invalid signature', async () => {
    ...
    const response = await request(app)
      .post('/webhooks/calls')
      .set('X-Tazzi-Signature', 'invalid-signature')
      .set('X-Tazzi-Timestamp', Math.floor(Date.now() / 1000).toString())
      .send(event);
  ...
  test('should reject old timestamps', async () => {
    ...
    const response = await request(app)
      .post('/webhooks/calls')
      .set('X-Tazzi-Signature', signature)
      .set('X-Tazzi-Timestamp', oldTimestamp.toString())
      .send(event);
...
def verify_signature(request):
    ...
    timestamp = request.headers.get('X-Tazzi-Timestamp')
...
function verifySignature($request) {
    $signature = $_SERVER['HTTP_X_TAZZI_SIGNATURE'] ?? '';
    $timestamp = $_SERVER['HTTP_X_TAZZI_TIMESTAMP'] ?? '';
...
sequenceDiagram
    ...
    Tazzi->>Webhook Endpoint: POST /webhooks (signed)
