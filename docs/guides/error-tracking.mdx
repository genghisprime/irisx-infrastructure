...
description: 'Monitor and debug production errors across Tazzi platform'
---

## Overview

Tazzi uses Sentry for real-time error tracking and performance monitoring across all applications. This guide covers setup, integration, and best practices.
...
|---------|-------------------------|---------|
| tazzi-api | `SENTRY_DSN_API` | Backend API errors |
| tazzi-customer-portal | `SENTRY_DSN_PORTAL` | Customer dashboard errors |
| tazzi-agent-desktop | `SENTRY_DSN_AGENT` | Agent interface errors |
...
export function initSentry() {
  ...
  Sentry.init({
    ...
    // Release tracking
    release: `tazzi-api@${process.env.npm_package_version || 'unknown'}`,
...
export function initSentry(app, router) {
  ...
  Sentry.init({
    ...
    // Release tracking
    release: `tazzi-customer-portal@${import.meta.env.VITE_APP_VERSION}`,
...
{
  "scripts": {
    "build": "tsc && sentry-cli sourcemaps upload --org=tazzi --project=api ./dist"
  }
}
```
