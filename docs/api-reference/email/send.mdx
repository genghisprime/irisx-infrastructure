...
Send transactional or marketing emails with delivery, open, and click tracking. Tazzi supports both plain text and HTML emails.
...
async function sendEmail(to, subject, bodyHtml, bodyText) {
  const response = await axios.post(
    'https://api.tazzi.com/v1/email/send',
    {
      to,
      from: 'noreply@tazzi.com',
      subject,
