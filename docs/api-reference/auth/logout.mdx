...
```bash cURL
curl -X POST https://api.useiris.com/v1/auth/logout \
...
async function logout() {
  try {
    ...
    await axios.post(
      'https://api.useiris.com/v1/auth/logout',
      {
...
function logout($token, $refreshToken) {
    $ch = curl_init('https://api.useiris.com/v1/auth/logout');
...
2. Contact IRISX support to manually revoke all tokens for an account

<Warning>
  If you suspect your API key or tokens have been compromised, immediately contact support@useiris.com to revoke all credentials.
</Warning>
