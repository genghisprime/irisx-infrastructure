...
```bash cURL
curl -X POST https://api.useiris.com/v1/auth/login \
...
async function login(email, password) {
  try {
    const response = await axios.post(
      'https://api.useiris.com/v1/auth/login',
      {
        email,
        password
      }
...
function login($email, $password) {
    $ch = curl_init('https://api.useiris.com/v1/auth/login');
...
<Tip>
  Implement rate limiting on your login endpoint to prevent brute force attacks. IRISX enforces 5 login attempts per minute per IP address.
</Tip>
