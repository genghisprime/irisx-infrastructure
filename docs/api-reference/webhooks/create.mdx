...
Create a webhook endpoint to receive real-time notifications for call, SMS, and email events. Tazzi will POST event data to your URL with HMAC signature verification.
...
```bash cURL
curl -X POST https://api.tazzi.com/v1/webhooks/endpoints \
...
async function createWebhook(url, events, description) {
  const response = await axios.post(
    'https://api.tazzi.com/v1/webhooks/endpoints',
    {
      url,
      events,
      description
...
Tazzi sends POST requests with this format:
