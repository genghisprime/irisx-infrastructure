---
title: 'Voice Calls'
description: 'Understanding voice calls in Tazzi'
---

## Overview

Tazzi provides enterprise-grade voice calling capabilities powered by FreeSWITCH with intelligent carrier routing, IVR menus, call recording, and real-time analytics.

## Call Lifecycle

Every call in Tazzi goes through several states:
...
Tazzi automatically selects the best carrier for each call using **LCR (Least Cost Routing)**.
...
If a call fails with the primary carrier, Tazzi automatically retries with the next-best carrier:
...
Tazzi supports both inbound and outbound calls:
...
Received on your Tazzi phone numbers:

```javascript
// Configure webhook on phone number
await tazzi.phoneNumbers.update(phoneNumberId, {
...
```javascript
// Get call details
const call = await tazzi.calls.get(callUuid);
...
Tazzi monitors call quality metrics:
