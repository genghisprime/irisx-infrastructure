---
title: 'Webhooks'
description: 'Receive real-time event notifications from IRISX'
---

## Overview

Webhooks allow your application to receive real-time notifications when events occur in IRISX (calls completed, messages delivered, etc.).
...
sequenceDiagram
    ...
    participant IRISX as IRISX API
    participant Webhook as Your Webhook

    App->>IRISX: Create call
    IRISX->>App: 201 Created
    IRISX->>Webhook: call.ringing
    IRISX->>Webhook: call.answered
    IRISX->>Webhook: call.completed
```
...
function verifySignature(req) {
  ...
  const webhookSecret = process.env.IRISX_WEBHOOK_SECRET;
...
IRISX signs every webhook with HMAC-SHA256:
...
If your endpoint is down, IRISX retries:
...
https://abc123.ngrok.io/webhooks/irisx
```
...
```javascript
const testWebhook = await irisx.webhooks.test(webhookId);
