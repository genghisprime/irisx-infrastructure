---
title: 'SMS Messaging'
description: 'Send and receive SMS/MMS messages with Tazzi'
---

## Overview

Tazzi provides reliable SMS and MMS messaging with delivery tracking, templates, scheduling, and opt-out management powered by Twilio and Telnyx.
...
```javascript
const scheduled = await tazzi.sms.schedule({
  to: '+15551234567',
  from: '+15559876543',
...
// Cancel if needed
await tazzi.sms.cancelScheduled(scheduled.id);
```

## Opt-Out Management

Tazzi automatically handles STOP/START keywords:
...
const optOuts = await tazzi.sms.getOptOuts();
